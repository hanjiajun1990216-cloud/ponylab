...
test.describe("Experiments List", () => {
  ...
  test("page loads with heading and new record button", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "新建记录" }).first()).toBeVisible();
  });
  ...
  test("project filter buttons work", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Protein Expression/ })).toBeVisible();
  });
...
test.describe("Experiment Detail", () => {
  ...
  test("has 6 tabs", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "版本历史" }).nth(1)).toBeVisible();
  });
  ...
  test("ELN editor toolbar is visible", async ({ page }) => {
    ...
    const toolbar = page.locator('[class*="toolbar"], [role="toolbar"], .tiptap-toolbar, [class*="menu-bar"]');
