...
test.describe("Dashboard Navigation", () => {
  ...
  test("dashboard displays stats and navigation", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /sample/i }).first()).toBeVisible();
    await expect(page.getByRole("link", { name: /inventor/i }).first()).toBeVisible();
  });
  ...
  test("navigate to inventory page", async ({ page }) => {
    await page.getByRole("link", { name: /inventor/i }).first().click();
    await expect(page).toHaveURL(/inventory/);
  });

  test("navigate to protocols page", async ({ page }) => {
    await page.getByRole("link", { name: /protocol/i }).first().click();
    await expect(page).toHaveURL(/protocols/);
  });

  test("navigate to instruments page", async ({ page }) => {
    await page.getByRole("link", { name: /instrument/i }).first().click();
